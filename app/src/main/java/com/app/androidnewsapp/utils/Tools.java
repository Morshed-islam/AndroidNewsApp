...
public class Tools {
    ...
    public static void validate(Activity activity) {
        SharedPref sharedPref = new SharedPref(activity);
        if (sharedPref.getItemId().equals(ITEM_ID) && sharedPref.getItemName().equals(ITEM_NAME)) {
        } else {
            new AlertDialog.Builder(activity)
                    ...
                    .setPositiveButton("Buy this item", (dialog, which) -> {
                        activity.finish();
                        activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://codecanyon.net/item/android-news-app/10771397")));
                    })
                    .setNegativeButton("Later", (dialog, which) -> activity.finish())
                    .setCancelable(false)
                    .show();
        }
    }
